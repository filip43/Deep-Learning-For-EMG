...
from aux_code.ops import one_hot_sequence, one_hot_encoder
...
def get_emg():
    combined = sio.loadmat('combined.mat')
    ...
    for gesture in range(0, no_of_classes-1):
        ...
        temp = np.append(y_train, np.full((8, 1), gesture))
        y_train.extend(temp)
        


        x_valid.extend(all_x[8:9])
        y_valid = np.append(y_valid, np.full((1, 1), gesture))
        y_valid_hot = one_hot_sequence(y_valid.astype(int)[:,None], no_of_classes-1)


        x_test.extend(all_x[9::])
        print(len(x_test))
        y_test = np.append(y_test, np.full((3, 1), gesture))
        y_test_hot = one_hot_sequence(y_test.astype(int)[:,None], no_of_classes-1)

    y_train_hot = one_hot_encoder(np.asarray(y_train), 10)

    return x_train, y_train_hot, x_valid, y_valid_hot, x_test, y_test_hot
