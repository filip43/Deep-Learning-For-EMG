...
def load_dataset():
    #Load the emg data
    XData = sio.loadmat('zeroGestureS1E2/emgShuffled.mat')
    ...
    #Load the labels
    YData = sio.loadmat('zeroGestureS1E2/yShuffled.mat')
...
def model(X_train, Y_train, X_test, Y_test, learning_rate,
          num_epochs, minibatch_size, print_cost, layer1Size, layer2Size, layer3Size, keep_probability):
    """
    ...
    with tf.Session(config=conf) as sess:
        ...
        for epoch in range(num_epochs):
            ...
            epoch_cost = 0.05                       # Defines a cost related to an epoch
...
parameters = model(X_train, Y_train, X_test, Y_test, learning_rate = 0.001,
          num_epochs = 600, minibatch_size = 256, print_cost = True, layer1Size=256, layer2Size=256, layer3Size = 128, keep_probability = 0.65)
end =time.time()
